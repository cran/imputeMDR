.packageName<-"imputeMDR"

`impute.mdr` <-
function(dataset, colresp, cs, combi, cv.fold = 10, na.method=0, randomize = FALSE) {
      ...
      errRate2.C <- function(comb, train, test, threshold, na_method, loc_na, loc_target, target_num) {
        z <- .C("err_rate", as.integer(comb), as.integer(nrow(comb)), as.integer(ncol(comb)),
                ...
                as.integer(loc_na-1), as.integer(loc_target-1), as.integer(target_num), length(loc_na), NAOK = FALSE,PACKAGE="imputeMDR")
   ...
   if (sum(a)>0)
    ...
    for (i in 1:cv.fold) {
      ...
      errs <- errRate2.C(comb, train, test, threshold, na_method=na.method, loc_na, loc_target, target_num)
